...
export function useAIAdvisorsDropdown() {
  ...
  const safeAIAdvisors = Array.isArray(aiAdvisors)
    ? aiAdvisors.filter(
  ...
  const configuredAIAdvisors = safeAIAdvisors.filter(
    (advisor) => advisor && advisor.apiKey && advisor.apiKey.length > 0,
  );
  ...
  useEffect(() => {
    if (configuredAIAdvisors.length > 0 && !selectedAIAdvisorId) {
      try {
        ...
        const advisorWithApiKey = configuredAIAdvisors.find(
          (advisor) => advisor && advisor.apiKey && advisor.apiKey.length > 0,
        );
        ...
        console.error("Error in auto-selection of AI advisor:", error);
      }
    }
  ...
  const handleAIAdvisorSave = useCallback(
    (provider: LLMProvider, apiKey: string, modelId?: string, personality?: Personality) => {
      ...
      try {
        ...
        if (newAIAdvisor && newAIAdvisor.id) {
  ...
  const handleAIAdvisorRemove = useCallback(
    (id: string) => {
      ...
      try {
        ...
        const aiAdvisor = safeAIAdvisors.find((m) => m && m.id === id);
        ...
        if (id === selectedAIAdvisorId) {
          ...
          const nextAIAdvisor =
            configuredAIAdvisors.find((m) => m && m.id !== id && m.apiKey && m.apiKey.length > 0) ||
            configuredAIAdvisors.find((m) => m && m.id !== id);

          if (nextAIAdvisor && nextAIAdvisor.id) {
  ...
  const handleAIAdvisorSelect = useCallback(
    (id: string) => {
      ...
      try {
        ...
        console.error("Error in handleAIAdvisorSelect:", error);
        return false;
      }
    },
