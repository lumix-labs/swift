  ...
  for (const symbol of Object.values(symbols)) {
    if ((symbol.type === 'function' || symbol.type === 'method') && symbol.children.length > 0) {
      for (const child of symbol.children) {
        if (
          symbols[child] &&
          (symbols[child].type === 'function' || symbols[child].type === 'method')
        ) {
          edges.push({
            source: symbol.name,
            target: child,
...
function createComponentHierarchy(symbols: Record<string, CodeSymbol>): HierarchyData {
  ...
  function ensurePath(parts: string[]): Record<string, TreeNode> {
    ...
    for (const part of parts) {
      ...
      // Get the next node - guaranteed to be a TreeNode
      const nextNode = current[part];
      // We need to cast nextNode to Record<string, TreeNode> to satisfy TypeScript
      current = nextNode as unknown as Record<string, TreeNode>;
    }
