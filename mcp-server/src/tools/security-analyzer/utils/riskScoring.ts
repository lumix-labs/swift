  ...
  const overallRiskScore = Math.min(
    ...
    Math.round(
      ((severityCounts.critical * 10 +
        ...
        Math.max(1, allVulnerabilities.length)) *
        10
    )
  );
 ...
 */
function calculateCategoryRiskScores(
  vulnerabilities: VulnerabilityItem[]
): Record<string, CategoryRiskScore> {
  // Define categories
  const categories = [
    'injection',
    'authentication',
  ...
  for (const category of categories) {
    ...
    // Count by severity
    const criticalCount = categoryVulns.filter(
    ...
    const score = Math.min(
      100,
      Math.round((criticalCount * 10 + highCount * 5 + mediumCount * 2 + lowCount * 1) * 5)
    );
