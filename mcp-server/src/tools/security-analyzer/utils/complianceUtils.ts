...
function getPciRequirement(vulnerability: VulnerabilityItem): RequirementInfo {
  ...
  const categoryToRequirement: RequirementMapping = {
    authentication: {
      ...
      details:
        'Requires unique authentication credentials for all users and secure authentication mechanisms.',
    },
    authorization: {
      id: '7.1',
      description: 'Limit access to system components',
      details:
        'Access to system components and data should be limited to only those individuals whose job requires such access.',
    },
    'data-protection': {
      id: '3.4',
      description: 'Render PAN unreadable anywhere it is stored',
      details:
        'Render Primary Account Numbers (PANs) unreadable using methods such as strong one-way hash functions, truncation, etc.',
    },
    'secrets-management': {
      id: '6.5.3',
      description: 'Protect credentials in code',
      details:
        'Develop applications based on secure coding guidelines to protect sensitive credentials in code.',
    },
    // Default case
    default: {
      id: '6.5',
  ...
  return (
    categoryToRequirement[vulnerability.category as keyof RequirementMapping] ||
    categoryToRequirement['default']
  );
}
...
function getGdprRequirement(vulnerability: VulnerabilityItem): RequirementInfo {
  ...
  const categoryToRequirement: RequirementMapping = {
    ...
    privacy: {
      ...
      details:
        'Implement appropriate technical and organizational measures for data protection by design and by default.',
    },
    // Default case
    default: {
      id: 'Article 5',
