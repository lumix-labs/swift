...
export function filterResultsBySeverity(results: SecurityAnalysisResults, threshold: string): void {
  ...
  if (results.dependencyVulnerabilities) {
    results.dependencyVulnerabilities = results.dependencyVulnerabilities.filter(
      (vuln: VulnerabilityItem) => {
        const vulnLevel =
          severityLevels[vuln.severity.toLowerCase() as keyof typeof severityLevels];
        return vulnLevel >= thresholdLevel;
      }
    );
  }
  ...
  if (results.hardcodedCredentials) {
    results.hardcodedCredentials = results.hardcodedCredentials.filter(
      (cred: VulnerabilityItem) => {
        const credLevel =
          severityLevels[cred.severity.toLowerCase() as keyof typeof severityLevels];
  ...
  if (results.securityAntiPatterns) {
    results.securityAntiPatterns = results.securityAntiPatterns.filter(
      (pattern: VulnerabilityItem) => {
        const patternLevel =
          severityLevels[pattern.severity.toLowerCase() as keyof typeof severityLevels];
