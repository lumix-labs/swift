...
export interface VulnerabilityItem {
  ...
  // Properties used in mapGenerator.ts
  isEntryPoint?: boolean;
  isSink?: boolean;
  // Properties used in riskScoring.ts
...
export interface ComplianceReport {
  standard?: string; // Add standard property
  version?: string;
  compliant: boolean;
  summary: string;
  failedRequirements: FailedRequirement[];
  passedRequirements?: FailedRequirement[]; // Add passedRequirements property
}
...
export interface FailedRequirement {
  ...
  vulnerability?: VulnerabilityItem; // Add missing property
}
...
export function formatSecurityResults(results: SecurityAnalysisResults): string {
  ...
  if (results.codeVulnerabilities && results.codeVulnerabilities.length > 0) {
    output += formatVulnerabilitiesSection('Code Vulnerabilities', results.codeVulnerabilities);
  }
  ...
  if (results.hardcodedCredentials && results.hardcodedCredentials.length > 0) {
    output += formatVulnerabilitiesSection('Hardcoded Credentials', results.hardcodedCredentials);
  }

  // Add security anti-patterns section
  if (results.securityAntiPatterns && results.securityAntiPatterns.length > 0) {
    output += formatVulnerabilitiesSection('Security Anti-Patterns', results.securityAntiPatterns);
  }
...
function formatVulnerabilitiesSection(title: string, vulnerabilities: VulnerabilityItem[]): string {
  ...
  for (const severity of severities) {
    const vulnsWithSeverity = vulnerabilities.filter((v) => v.severity.toLowerCase() === severity);

    if (vulnsWithSeverity.length > 0) {
      output += `#### ${capitalize(severity)} Severity (${vulnsWithSeverity.length})\n\n`;

      for (const vuln of vulnsWithSeverity.slice(0, 10)) {
...
function formatRemediationSection(remediationPriority: RemediationItem[]): string {
  ...
  output +=
    'The following vulnerabilities should be addressed first based on severity, exploitability, and impact:\n\n';
